      ...
      <div className="flex flex-col px-70 py-20 backdrop-blur-sm">
        ...
        <div className="mt-10">
          {data.map((product, index) => (
            <div
              key={index}
              className=" my-10 not-first:my-40 flex even:flex-row-reverse items-center justify-between relative"
            >
                  ...
                  <div className="mt-[70px] space-y-2 text-secondary3 font-Lato">
                    {product.prices?.map((price, index) => (
                      <div key={index} className="flex mt-4">
                        ...
                        <p className="ml-[70px] font-bold">${price.Price}</p>
                      </div>
                    ))}
                  </div>
                </div>
                <div className="-mt-5">
                  <Image width={600} src={images.menupaper} alt="menu" />
                </div>
              </div>
              <div className="w-[370px] flex relative  h-[400px]">
                <Link
                  href={"/Gallery"}
